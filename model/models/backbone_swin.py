"""
SWIN CODE MODIFIED FROM MONAI GITHUB: https://github.com/Project-MONAI/MONAI
...
def custom_Swin(config, input_feature_channels):
    ...
    model = SwinTransformer_with_alt_ops(use_hyena=config.Swin.use_hyena,
                                        use_mamba=config.Swin.use_mamba,
                                        in_chans=input_feature_channels,
                                        embed_dim=embed_dim,
...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        if not self.use_hyena and not self.use_mamba:
            ...
            self.hyena = HyenaOperator(d_model=self.dim,
                                        l_max=66000,
                                        filter_order=64,
                                        num_heads=num_heads,
                                        num_blocks=1,
                                        ...
                                        bidrectional=False,
                                        dropout=attn_drop,
                                        filter_dropout=proj_drop,
                                        activation="id"
            )
        elif not use_hyena and use_mamba:
            self.mamba = MambaVisionMixer(d_model=self.dim, 
                                          d_state=8,  
                                          d_conv=3,    
                                          expand=1
                                          )
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.window_size = window_size
        if use_hyena or use_mamba: self.shift_size = tuple(0 for i in window_size)
        else: self.shift_size = tuple(i // 2 for i in window_size)
...
class SwinTransformer_with_alt_ops(nn.Module):
    """
